    ...
    >
      ...
      <div className="rv-glass rounded-[2rem] px-8 py-6">
        <div className="mb-4">
          <span
            ...
            style={{ background: 'rgba(167,139,250,0.07)', color: '#a78bfa', border: '1px solid rgba(167,139,250,0.18)' }}
          >
            🔮 Synonym Drill
          </span>
        </div>
        <motion.h2
          ...
          style={{
            ...
            textShadow: '0 0 32px rgba(167,139,250,0.25)',
            letterSpacing: '-0.01em',
          }}
        >
          {currentItem.word.word}
        </motion.h2>
        <p className="text-zinc-500 text-xs mt-2">Tap all the synonyms. Watch out for imposters.</p>
      </div>
      ...
      <AnimatePresence>
        {allDone && (
          ...
          >
            <div>
              <p className="text-[10px] uppercase tracking-widest font-bold text-zinc-500 mb-1">Words you can swap in</p>
              <p className="text-sm text-zinc-400">
                All synonyms for <span className="text-white font-semibold">{currentItem.word.word}</span>.
              </p>
            </div>
            <div className="space-y-2">
              {synonyms.map((syn, i) => (
                ...
                >
                  ...
                  <span className="text-sm font-semibold" style={{ color: '#a78bfa', fontFamily: "'Space Grotesk', sans-serif" }}>
                    {syn}
                  </span>
                </motion.div>
              ))}
            ...
            <motion.button
              ...
              transition={{ delay: synonyms.length * 0.07 + 0.1 }}
      ...
      {!allDone && (
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-zinc-600 shrink-0 ml-3">
              {foundCount} / {synonyms.length}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {tiles.map((tile, i) => {
              ...
              const isCorrectTile = state === 'correct';
              ...
              return (
                <motion.button
                  ...
                  animate={{
                    opacity: 1,
                    scale: 1,
                    x: isWrong ? [-5, 5, -4, 4, -2, 2, 0] : 0,
                  }}
                  transition={{
                    opacity:  { delay: i * 0.06, duration: 0.2 },
                    scale:    { delay: i * 0.06, duration: 0.2 },
                    x:        { duration: 0.38 },
                  }}
                  whileTap={!isCorrectTile ? { scale: 0.94 } : {}}
                  className="relative rounded-2xl px-4 py-4 text-sm font-bold text-left"
                  style={{
                    background: isCorrectTile ? 'rgba(0,255,200,0.12)' : isWrong ? 'rgba(239,68,68,0.12)' : 'rgba(139,92,246,0.08)',
                    border: `1px solid ${isCorrectTile ? 'rgba(0,255,200,0.45)' : isWrong ? 'rgba(239,68,68,0.45)' : 'rgba(139,92,246,0.2)'}`,
                    boxShadow: isCorrectTile ? '0 0 18px rgba(0,255,200,0.18)' : 'none',
                    color: isCorrectTile ? '#00FFC8' : isWrong ? '#ef4444' : '#d4d4d8',
                    cursor: isCorrectTile || gridDone ? 'default' : 'pointer',
